...
- [Cloudflare Workers 详细说明](#cloudflare-workers-详细说明)
  ...
  - [部署指南](#部署指南)
  - [本地调试与测试](#本地调试与测试)
  - [从其他方式迁移到 CF Workers](#从其他方式迁移到-cloudflare-worker)
  - [费用](#费用)
  - [CF Workers 常见问题](#cloudflare-worker-常见问题)
- [常见问题](#常见问题)
- [安全提醒](#安全提醒)
- [来源与鸣谢](#来源与鸣谢)
- [贡献](#贡献)
...
- 12 种推送通道：Server 酱、PushPlus、Telegram、Discord、Wecom 酱 / 企业微信应用、企业微信群机器人、WxPusher、Bark、钉钉、飞书、ntfy、Gotify
...
```bash
git clone https://github.com/adrian803/roco-serverchan-notifier.git
cd roco-serverchan-notifier
cp .env.example .env
```
...
5. Worker 名称需要和 [cloudflare-worker/wrangler.toml](cloudflare-worker/wrangler.toml) 中的 `name` 一致，默认为 `roco-serverchan-notifier-worker`
...
| `2` / `worker-js` | **_worker.js 直接部署**：直接发布仓库内现有 `cloudflare-worker/_worker.js`，不重新生成 | 只想验证或发布控制台粘贴版产物 |
...
脚本会隐藏提示输入 `CLOUDFLARE_API_TOKEN`。选择配置 secrets 后，会依次提示输入 `ROCOM_API_KEY`、`SERVERCHAN_SENDKEY` 和可选 `TRIGGER_TOKEN`，并通过 `wrangler secret put` 写入 Cloudflare，不会写入仓库文件。交互模式还允许临时修改 Worker 名称；默认仍读取 `cloudflare-worker/wrangler.toml`。
...
| `INCLUDE_PRICE_INFO` | 推送商品价格和限购数量；未收录价格显示 `价格未收录` | `true` / `false` |

可用通道 ID：`serverchan-default`、`pushplus-env`、`telegram-env`、`discord-env`、`wecomchan-env`、`wecom-bot-env`、`wxpusher-env`、`bark-env`、`dingtalk-env`、`feishu-env`、`ntfy-env`、`gotify-env`。
...
控制台粘贴部署在 Cloudflare Dashboard → Worker → Settings → Variables and Secrets 中配置。Wrangler 项目部署通过 `wrangler secret put` 配置 secrets，非敏感变量放在 `cloudflare-worker/wrangler.toml`。必需配置 `ROCOM_API_KEY` 和至少一个推送通道 secret；可选配置 `TRIGGER_TOKEN` 保护 `/trigger` 端点。详见 [环境变量参考](#secrets敏感字段)。

## 推送通道

12 种推送通道，Docker 版和 CF Workers 版完全一致：
...
| `CONSOLE_PASSWORD` | 空 | 控制台密码；为空时首次启动生成强密码，明文打印到日志，哈希保存到 `./data/config.json` |
...
| `INCLUDE_PRICE_INFO` | `false` | 推送商品时附带限购数量、单价和合计洛克贝；未收录价格显示 `价格未收录` |
...
# 启动自动模式
uv run python -m roco_serverchan_notifier.launcher
...
# 构建镜像
docker build -t roco-serverchan-notifier:latest .
```
...
完整维护说明见 [DEVELOPMENT.md](DEVELOPMENT.md)，包含模块边界、provider manifest 规范、新增推送通道步骤、Python/Worker 双端同步规则、测试矩阵和发布检查清单。
...
可选添加 Repository Variable：`PUSHPLUS_TOKEN`、`TELEGRAM_CHAT_ID`、`DELIVERY_MODE`、`NOTIFY_EMPTY`、`INCLUDE_PRICE_INFO`、`HTTP_TIMEOUT` 等。
...
CF Workers 版将核心推送逻辑打包为单文件 [cloudflare-worker/_worker.js](cloudflare-worker/_worker.js)，使用 Cloudflare Cron Triggers 定时执行。支持四种入口：控制台粘贴 `_worker.js`、Workers Builds 从 GitHub/GitLab 项目部署、一键脚本部署，或本地 Wrangler 部署。四种方式使用同一份源码，`_worker.js` 是从 `src/` 构建出的产物。
...
├── test/
│   ├── worker.test.ts         # Worker 行为测试
│   ├── push-modules.test.ts   # 推送模块边界测试
│   └── cross-runtime.test.ts  # Python/Worker 一致性 fixture 测试
└── src/
    ...
    ├── push-providers.ts      # 推送通道校验、分发和脱敏门面
    ├── push-provider-senders/ # 按 chat/token/webhook/企业微信拆分的通道实现
    ├── push-http.ts           # 通用 HTTP 响应解析与错误结果
    ├── push-provider-auth.ts  # 企业微信 token、钉钉/飞书签名
    ├── push-redaction.ts      # 推送错误与响应脱敏
    └── provider-specs.ts      # 从共享 manifest 读取通道规格
```
...
| `settings.py` / `env_providers.py` / `config_store.py` / `config.py` | `config.ts` | 环境变量装配、设置对象、持久化 |
...
| `push.py` / `push_delivery.py` / `push_providers.py` / `push_provider_senders/` / `push_redaction.py` | `push.ts` / `push-delivery.ts` / `push-providers.ts` / `push-provider-senders/` / `push-http.ts` / `push-provider-auth.ts` / `push-redaction.ts` | 通道实现、HTTP 公共逻辑、鉴权签名、投递引擎、脱敏 |
| `provider_specs.py` / `shared/provider_manifest.json` | `provider-specs.ts` | 共享通道规格、env 映射、默认值 |
| `web_auth.py` / `console_password.py` / `console_session.py` | - | 控制台密码、首次生成、session cookie |
| `scheduler.py` / `schedule_policy.py` / `scheduler_state.py` | - | 调度运行循环、时间策略、状态模型 |
...
```bash
# 健康检查
curl https://roco-serverchan-notifier-worker.<子域>.workers.dev/

# 开放模式
curl https://roco-serverchan-notifier-worker.<子域>.workers.dev/trigger

# 配置 TRIGGER_TOKEN 后任选一种
curl "https://roco-serverchan-notifier-worker.<子域>.workers.dev/trigger?token=你的token"
curl -H "X-Trigger-Token: 你的token" https://roco-serverchan-notifier-worker.<子域>.workers.dev/trigger
curl -H "Authorization: Bearer 你的token" https://roco-serverchan-notifier-worker.<子域>.workers.dev/trigger
```
...
- 自动化项目部署：用 Workers Builds 连接 GitHub/GitLab，Root directory 设为 `cloudflare-worker`，Deploy command 设为 `npm run deploy`。
...
- 发布 Release 时：`worker-release.yml` 会重新跑 Worker 测试和 bundle 检查，并把 `cloudflare-worker/_worker.js` 上传到 GitHub Release 附件。
...
**如何更新到最新版本？** 控制台粘贴部署：复制仓库或 GitHub Release 附件中的新版 [_worker.js](cloudflare-worker/_worker.js) 全量内容，替换 Worker 编辑器中的代码并保存部署。Workers Builds：推送到生产分支后自动部署。一键脚本部署：`git pull` 后运行 `.\scripts\deploy-cf-worker.ps1` 或 `bash scripts/deploy-cf-worker.sh`。手动 Wrangler 项目部署：`git pull && cd cloudflare-worker && npm ci && npm run deploy`。
...
**CF Workers 版：** 手动触发 `curl <worker-url>/trigger`，查看返回结果。若配置了 `TRIGGER_TOKEN`，使用 `curl '<worker-url>/trigger?token=你的token'`。执行日志优先在 Cloudflare Dashboard → Workers → 你的 Worker → Logs 查看；Wrangler 项目部署也可以在 `cloudflare-worker` 目录运行 `npm run tail`。
...
```bash
docker exec -u root roco-serverchan-notifier chown -R app:app /data
```
...
如果使用 `APP_MODE=web`，或因缺少 Key 进入控制台模式，控制台监听 `0.0.0.0` 并发布到宿主机 `19892` 端口。`CONSOLE_PASSWORD` 为空时会在首次启动生成默认强密码，明文打印到日志，哈希保存到 `./data/config.json` 供后续重启复用。

**公开部署前请务必：**

- 保存首次启动日志里的随机密码，或在 `.env` 设置自定义强密码：`CONSOLE_PASSWORD=你的密码`
- 只在可信网络开放 `19892`，或用防火墙 / 反向代理限制访问
- 本机访问改为 `127.0.0.1:${WEB_PORT:-19892}:19892`
- 仅本机临时调试时才使用 `CONSOLE_ALLOW_EMPTY_PASSWORD=true`
- 不要提交 `./data/config.json`（含明文 token、Key 和控制台密码哈希）

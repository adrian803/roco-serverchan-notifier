...
这份文档面向后续维护者，说明仓库结构、核心边界、推送通道扩展方式、双端同步规则和发布前检查。使用者部署请看 [README.md](README.md)，这里主要讲“怎么改代码不漂”。
...
src/roco_serverchan_notifier/
  ...
  push_provider_senders/       按 chat/token/webhook/企业微信拆分的发送器
...
- README 面向使用者，DEVELOPMENT 面向维护者。部署步骤改动通常要同步 README；架构、扩展流程或内部边界改动要同步 DEVELOPMENT。
...
- `chat.*`：Telegram、Discord 这类聊天平台文本推送。
- `token.*`：Server 酱、PushPlus、WxPusher、Bark、ntfy、Gotify。
- `webhook.*`：钉钉、飞书。
- `wecom.*`：企业微信应用和群机器人。
